...
class TestQuaternion(unittest.TestCase):
    ...
    def test_addition(self):
        q1 = pyvek.quat(1.0, 2.0, 3.0, 4.0)
        q2 = pyvek.quat(5.0, 6.0, 7.0, 8.0)
        q3 = q1 + q2
    ...
    def test_subtraction(self):
        q1 = pyvek.quat(5.0, 6.0, 7.0, 8.0)
        q2 = pyvek.quat(1.0, 2.0, 3.0, 4.0)
        q3 = q1 - q2
    ...
    def test_dot_product(self):
        q1 = pyvek.quat(1.0, 0.0, 0.0, 0.0)
        q2 = pyvek.quat(0.0, 1.0, 0.0, 0.0)
        self.assertAlmostEqual(q1.dot(q2), 0.0)

        q3 = pyvek.quat(1.0, 2.0, 3.0, 4.0)
        q4 = pyvek.quat(5.0, 6.0, 7.0, 8.0)
    ...
    def test_dot_function(self):
        ...
        q2 = pyvek.quat(5.0, 6.0, 7.0, 8.0)
        expected = 1*5 + 2*6 + 3*7 + 4*8
        self.assertAlmostEqual(pyvek.dot(q1, q2), expected)
...
class TestQuaternionSlerp(unittest.TestCase):
    def test_slerp_identity_to_90deg(self):
        q1 = pyvek.quat(0, 0, 0, 1)  # Identity
        q2 = pyvek.quat(0, 0, 0.7071068, 0.7071068)  # 90 deg around Z

        # At t=0, should be q1
        q_start = pyvek.slerp(q1, q2, 0.0)
        self.assertAlmostEqual(q_start.w, 1.0, places=5)

        # At t=1, should be q2
        q_end = pyvek.slerp(q1, q2, 1.0)
    ...
    def test_slerp_class(self):
        q1 = pyvek.quat(0, 0, 0, 1)
        q2 = pyvek.quat(0, 0, 1, 0)

        slerper = pyvek.quat_slerper()
        slerper.setup(q1, q2)
    ...
    def test_slerp_preserves_norm(self):
        q1 = pyvek.quat(1, 0, 0, 0)
        q1.normalize()
        q2 = pyvek.quat(0, 1, 0, 0)
        q2.normalize()

        for t in [0.0, 0.25, 0.5, 0.75, 1.0]:
            q = pyvek.slerp(q1, q2, t)
...
class TestQuaternionMatrix(unittest.TestCase):
    def test_mat3_rotation_from_quat(self):
        ...
        q = pyvek.quat(0, 0, 0.7071068, 0.7071068)
        m = pyvek.mat3()
        m.rotation(q)
    ...
    def test_mat4_rotation_around_x(self):
        ...
        q = pyvek.quat(0.7071068, 0, 0, 0.7071068)
        m = pyvek.mat4()
        m.rotation(q)
    ...
    def test_mat4_rotation_around_y(self):
        ...
        q = pyvek.quat(0, 0.7071068, 0, 0.7071068)
        m = pyvek.mat4()
        m.rotation(q)
    ...
    def test_quaternion_rotation_composition(self):
        ...
        q45 = pyvek.quat(0, 0, 0.3826834, 0.9238795)  # 45 deg
...
class TestDoubleQuaternion(unittest.TestCase):
    ...
    def test_dquat_norm(self):
        q = pyvek.dquat(3.0, 4.0, 0.0, 0.0)
        self.assertAlmostEqual(q.norm(), 5.0)

    def test_dquat_slerp(self):
        slerper = pyvek.dquat_slerper()
        q1 = pyvek.dquat(0, 0, 0, 1)
        q2 = pyvek.dquat(0, 0, 1, 0)
        slerper.setup(q1, q2)
        q_mid = slerper.interpolate(0.5)
        self.assertAlmostEqual(q_mid.norm(), 1.0, places=5)
...
class TestNumpyConversions(unittest.TestCase):
    ...
    def test_vec2_roundtrip(self):
        v1 = pyvek.vec2(5.0, 6.0)
        arr = v1.to_numpy()
        v2 = pyvek.vec2.from_numpy(arr)
    ...
    def test_vec3_roundtrip(self):
        v1 = pyvek.vec3(7.0, 8.0, 9.0)
        arr = v1.to_numpy()
        v2 = pyvek.vec3.from_numpy(arr)
        self.assertEqual(v1.x, v2.x)
        self.assertEqual(v1.y, v2.y)
    ...
    def test_vec4_roundtrip(self):
        v1 = pyvek.vec4(9.0, 10.0, 11.0, 12.0)
        arr = v1.to_numpy()
        v2 = pyvek.vec4.from_numpy(arr)
        self.assertEqual(v1.x, v2.x)
        self.assertEqual(v1.y, v2.y)
        self.assertEqual(v1.z, v2.z)
    ...
    def test_dvec2_from_numpy(self):
        ...
        v = pyvek.dvec2.from_numpy(arr)
        self.assertEqual(v.x, 1.0)
        self.assertEqual(v.y, 2.0)

    def test_dvec3_from_numpy(self):
        arr = np.array([1.0, 2.0, 3.0], dtype=np.float64)
        v = pyvek.dvec3.from_numpy(arr)
        self.assertEqual(v.x, 1.0)
        self.assertEqual(v.y, 2.0)
        self.assertEqual(v.z, 3.0)
